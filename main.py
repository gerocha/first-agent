...
SYSTEM_PROMPT = """Answer the following questions as best you can. You have
access to the following tools:
    get_weather: Get the current weather in a given location

    The way you use the tools is by specifying a json blob.
    Specially, this json shoud have an `action` key (with the name of the tool
    to use) and an `action_input` key (with the input to the tool going here).
...
output = client.chat.completions.create(
        messages=[
            ...
            {"role": "user", "content": "What's the weather in Praia Grande?"},
            ],
        stream=False,
        max_tokens=150,
